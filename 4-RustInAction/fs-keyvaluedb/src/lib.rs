...
impl ActionKV {
    ...
    //assumes that f is the right place in the file
    // f can be any type that impl Read
    fn process_record<R: Read>(f: &mut R) -> io::Result<KeyValuePair> {
        // each kv pair is prefixed by 12 bytes (length and checksuum)
    ...
    pub fn get(&mut self, key: &ByteStr) -> io::Result<Option<ByteString>> { // Wraps Option within Result to allow for the possibility of an I/O error as well as tolerating missing values
    ...
    // update basically add a new entry at the end
    #[inline]
    pub fn update(&mut self, key: &ByteStr, value: &ByteStr) -> io::Result<()> {
        self.insert(key, value)
    }

    // delete add a new entry at the end with null value
    #[inline]
