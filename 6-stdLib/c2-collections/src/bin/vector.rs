fn main() {
    ...
    let second = fruits.get(1); // get=borrow elem / get_mut (mutate it)
    ...
    names.retain(|name| name.starts_with('A')); // uses a PREDICATE (function thar eturns true or false)
    ...
    for letter in alphabet.drain(..2) { // creates iterator over vector, removing elements iterated
        print!("{} ", letter);
    }
    println!();
    ...
    // Splice a vector. Do 3 things
    // 1. Takes a range (will be removed from the vector)
    // 2. Takes an iterator: will be inserted into the space left open by removal in 1
    // 3- Returns removed elements as an iterator
